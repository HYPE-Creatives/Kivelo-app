// app/(dashboard)/parent/chat.tsx - Parent-Child Direct Chat Screen
...
export default function ParentChatScreen() {
  ...
  const initializeChat = async () => {
    setLoading(true);
    try {
      // Create or get existing conversation with this child
      const conversation = await createFamilyChat([childId], 'parent_child');
      if (conversation) {
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
          ...
          >
            ...
            <View style={styles.headerInfo}>
              {childAvatar ? (
                <Image source={{ uri: childAvatar }} style={styles.headerAvatar} />
              ) : (
                <View style={[styles.headerAvatarPlaceholder, { backgroundColor: 'rgba(255,255,255,0.2)' }]}>
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
          ...
          >
            ...
            <View style={styles.headerInfo}>
              ...
              <View>
                <Text style={styles.headerName}>{childName}</Text>
                <Text style={styles.headerStatus}>Private Chat</Text>
              </View>
            </View>
          ...
          >
            {messages.length === 0 ? (
              <View style={styles.emptyChat}>
                <View style={[styles.emptyChatIcon, { backgroundColor: themeColors.primary + '15' }]}>
                  <Ionicons name="chatbubbles-outline" size={48} color={themeColors.primary} />
                </View>
                <Text style={[styles.emptyChatTitle, { color: colors.text }]}>
                  Start a conversation
                </Text>
                <Text style={[styles.emptyChatText, { color: colors.textSecondary }]}>
                  Send a message to {childName}. All messages are kept safe and private.
                </Text>
              </View>
            ) : (
                      ...
                      >
                        {!isMe && (
                          <View style={styles.messageAvatarContainer}>
                            {childAvatar ? (
                              <Image source={{ uri: childAvatar }} style={styles.messageAvatar} />
                            ) : (
                              <View style={[styles.messageAvatarPlaceholder, { backgroundColor: '#FF6B9D20' }]}>
                                <Ionicons name="person" size={12} color="#FF6B9D" />
                              </View>
                            )}
                          </View>
                        )}
          ...
          <View style={[styles.inputWrapper, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
            <View style={styles.inputContainer}>
              ...
              >
                <LinearGradient
                  colors={message.trim() && !isSending ? themeColors.gradient : ['#D1D5DB', '#D1D5DB']}
        ...
        >
          <View style={[styles.editModal, { backgroundColor: colors.card }]}>
            ...
            <View style={styles.editActions}>
              ...
              <TouchableOpacity 
                style={[styles.editSaveButton, { backgroundColor: themeColors.primary }]} 
